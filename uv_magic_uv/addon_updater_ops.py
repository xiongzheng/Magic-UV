...
class MUV_addon_updater_install_popup(bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        if updater.manual_only==True:
            ...
            atr = MUV_addon_updater_install_popup.bl_idname.split(".")
...
class MUV_addon_updater_check_now(bpy.types.Operator):
    bl_label = "Check now for Magic UV update"
...
class MUV_addon_updater_update_now(bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        if updater.update_ready == True:
            ...
            try:
                ...
                atr = MUV_addon_updater_install_manually.bl_idname.split(".")
                getattr(getattr(bpy.ops, atr[0]),atr[1])('INVOKE_DEFAULT')
        elif updater.update_ready == None:
            (update_ready, version, link) = updater.check_for_update(now=True)
            # re-launch this dialog
...
class MUV_addon_updater_update_target(bpy.types.Operator):
...
class MUV_addon_updater_install_manually(bpy.types.Operator):
...
class MUV_addon_updater_updated_successful(bpy.types.Operator):
...
class MUV_addon_updater_restore_backup(bpy.types.Operator):
...
class MUV_addon_updater_ignore(bpy.types.Operator):
...
class MUV_addon_updater_end_background(bpy.types.Operator):
...
def updater_run_success_popup_handler(scene):
    ...
    atr = MUV_addon_updater_updated_successful.bl_idname.split(".")
    getattr(getattr(bpy.ops, atr[0]),atr[1])('INVOKE_DEFAULT')
...
def update_notice_box_ui(self, context):
    ...
    if updater.manual_only==False:
        ...
        col.operator(MUV_addon_updater_update_now.bl_idname,
                        "Update now", icon="LOOP_FORWARDS")
        col.operator("wm.url_open", text="Open website").url = updater.website
        #col.operator("wm.url_open",text="Direct download").url=updater.update_link
        col.operator(MUV_addon_updater_install_manually.bl_idname, "Install manually")
    else:
        col.label("Update ready!",icon="ERROR")
        #col.operator("wm.url_open",text="Direct download").url=updater.update_link
        col.operator("wm.url_open", text="Get it now").url = \
                updater.website
    col.operator(MUV_addon_updater_ignore.bl_idname, icon="X")
...
def update_settings_ui(self, context):
    ...
    if updater.error != None:
        ...
        split.enabled = False
        split.scale_y = 2
        split.operator(MUV_addon_updater_check_now.bl_idname,
					   updater.error)
        split = subcol.split(align=True)
        split.scale_y = 2
        split.operator(MUV_addon_updater_check_now.bl_idname,
        ...
        col.scale_y = 2
        col.operator(MUV_addon_updater_check_now.bl_idname)
        ...
        split = subcol.split(align=True)
        split.scale_y = 2
        split.operator(MUV_addon_updater_end_background.bl_idname,
					   text = "", icon="X")
        ...
        split = subcol.split(align=True)
        split.scale_y = 2
        split.operator(MUV_addon_updater_update_now.bl_idname,
    ...
    if updater.manual_only == False:
        ...
        if updater.include_branches == True and len(updater.include_branch_list)>0:
            ...
            col.operator(MUV_addon_updater_update_target.bl_idname,
        ...
        col.operator(MUV_addon_updater_restore_backup.bl_idname, backuptext)
...
def register(bl_info):
    ...
    updater.include_branch_list = ['master', 'develop']  # is the equivalent to setting ['master']
    ...
    bpy.utils.register_class(MUV_addon_updater_install_popup)
    bpy.utils.register_class(MUV_addon_updater_check_now)
    bpy.utils.register_class(MUV_addon_updater_update_now)
    bpy.utils.register_class(MUV_addon_updater_update_target)
    bpy.utils.register_class(MUV_addon_updater_install_manually)
    bpy.utils.register_class(MUV_addon_updater_updated_successful)
    bpy.utils.register_class(MUV_addon_updater_restore_backup)
    bpy.utils.register_class(MUV_addon_updater_ignore)
    bpy.utils.register_class(MUV_addon_updater_end_background)
...
def unregister():
    bpy.utils.unregister_class(MUV_addon_updater_install_popup)
    bpy.utils.unregister_class(MUV_addon_updater_check_now)
    bpy.utils.unregister_class(MUV_addon_updater_update_now)
    bpy.utils.unregister_class(MUV_addon_updater_update_target)
    bpy.utils.unregister_class(MUV_addon_updater_install_manually)
    bpy.utils.unregister_class(MUV_addon_updater_updated_successful)
    bpy.utils.unregister_class(MUV_addon_updater_restore_backup)
    bpy.utils.unregister_class(MUV_addon_updater_ignore)
    bpy.utils.unregister_class(MUV_addon_updater_end_background)
